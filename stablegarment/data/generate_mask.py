# from https://github.com/miccunifi/ladi-vton/blob/master/src/dataset/dresscode.py
...
if __name__=="__main__":
    ...
    os.makedirs("./agnostic-mask",exist_ok=True)
    os.makedirs("./agnostic",exist_ok=True)

    for index in range(len(dataset_train)):
        one_sample = dataset_train.__getitem__(index)
        mask = one_sample["inpaint_mask"]
        cv2.imwrite(f'./agnostic-mask/{one_sample["im_name"]}',np.array(mask[0]*255).astype(np.uint8))
        
        img = (np.array(one_sample["image"])+1.)*127.5
        img = np.array((1-mask)*img+mask*128).astype(np.uint8)
        cv2.imwrite(f'./agnostic/{one_sample["im_name"]}',img.transpose(1,2,0)[:,:,::-1])
